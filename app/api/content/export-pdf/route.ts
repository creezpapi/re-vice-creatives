...
<div class="deliv">
  ...
  ${d.filming_team?.length ? `<div class="meta"><span class="team-label">Filming team:</span> ${(d.filming_team as string[]).map(esc).join(', ')}</div>` : ''}
  ${d.editing_team?.length ? `<div class="meta"><span class="team-label">Editing team:</span> ${(d.editing_team as string[]).map(esc).join(', ')}</div>` : ''}
...
.meta a { color: #0066cc; }
.team-label { font-weight: 600; color: #333; }
